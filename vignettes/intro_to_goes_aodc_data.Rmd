...
measurement process can be found in the 
...
Aerosol optical depth is stored along with many other kinds of measurements
taken by the GOES-16 satellite in large netCDF "Common Data Form" files. These 
files are structured for the efficient access of multidimentional array-oriented
data like atmospheric readings. 

Our first step when using the `MazamaSatelliteUtils` package is to provide a 
`SatelliteDataDir`: A directory where all our working netCDF files will be 
...
A new netCDF file is generated about every five minutes, meaning that a single
hour is typically made up of around a dozen files. Downloading these files does 
not load them as objects in the R environment, but instead saves them to the 
`SatelliteDataDir` directory where they can be opened individually through their
netCDF handles.
...
be included in the returned vector). We can see which files are now availible to
read by calling `goesaodc_listFiles()` on the date we specified:
...
open the first one in our downloaded time range `dateFiles[1]`:
