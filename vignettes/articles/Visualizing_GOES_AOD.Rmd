...
`goesaodc_createScanPoints()` or rasters with `goesaodc_createScanRaster()`. 
...
`goesaodc_createScanPoints()` will return a list of SPDFs and 
`goesaodc_createScanRaster()` will return a RasterBrick.

```{r create_scan_series_points_and_rasterbrick}
...
```{r}
averagedScanPoints <- goesaodc_calcAverageScanPoints(
  spdfList = scanPointsList,
  na.rm = TRUE
)

averagedScanRaster <- goesaodc_calcAverageScanRaster(
...
trendScanPoints <- goesaodc_calcTrendScanPoints(
  spdfList = scanPointsList,
  na.rm = TRUE
)
...
At last, we can draw SPDFs/rasters with `goesaodc_plotScanPoints()` or
...
```{r plot_scan_points, warning=FALSE, message=FALSE, fig.width=7, fig.height=5, fig.align="center"}
# Load state polygons
...
```{r plot_aggregate_points_and_rasters, fig.width=7, fig.height=5, fig.align="center"}
goesaodc_plotScanPoints(
...
goesaodc_plotScanPoints(
  spdf = trendScanPoints,
  bbox = bboxOregon,
  legendTitle = "AOD trend"
)
```
