...
`starttime`), but require several additional arguments as well:

* `satelliteDataDir`: Path to the directory where satellite data is 
downloaded to and read from.
* `spatialDataDir`: Path to the directory where spatial data is downloaded to 
and read from.
* `outputDir`: Path to the directory where the video should be saved to.
* `logDir`: Path to the directory where the output log files should be saved to.
* `frameRate`: Framerate of the video in fps.
* `verbose`: Logical flag whether to print out progress messages in the console.
* `version`: Logical flag whether to print out the current version of the 
executable script.

Another thing to note: Since animations are made up of individual plots, it's
essential that they all share a consistent color legend because:

1. The color scale bounds shouldn't change between frames.
2. A legend should still be drawn even if all the AOD values for a frame are 
`NA`.

Therefore, the animation scripts _require_ that you set the `legendLimits`
parameter. This is enough to guarantee the two points above. 

Here's an some example of animating points:
...
And here's an example of animating rasters:
