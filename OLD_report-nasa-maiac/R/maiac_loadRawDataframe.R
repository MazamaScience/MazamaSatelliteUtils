...
maiac_loadRawDataframe <- function(tileNumber = NULL,
                                   date = NULL, 
                                   time = NULL,
                                   julianDate = NULL,
                                   product = "MAIACAAOT",
  ...
  hdffilePath <- maiac_downloadNorthAmerica(tileNumber, date, time, julianDate, product, baseUrl)
  ...
  toBitString <- function(x){ return( paste0( tail(rev(as.numeric(intToBits(x))),16) , collapse="") ) }
  ...
  # AOD uncertainty: This parameter is evaluated based on the Blue-band B3 surface brightness (reflectance) only, 
  # and thus gives only a general indication of possible increase of error over brighter surfaces;
