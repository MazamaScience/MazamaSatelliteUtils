...
maiac_downloadNorthAmerica <- function(tileNumber = NULL,
                                       date = NULL, 
                                       time = NULL,
                                       julianDate = NULL,
                                       product = "MAIACAAOT",
  ...
  # get dataDir
  maiacDataDir <- getMaiacDataDir()
  
  # Get time info
  if ( !is.null(date) ) {
    datetime <- PWFSLSmoke::parseDatetime(date)
  ...
  localMaiacFiles <- list.files(maiacDataDir)
  ...
  if ( length(matches) > 0 ) {
    ...
    filePath <- file.path(maiacDataDir, matches)
  } else {
