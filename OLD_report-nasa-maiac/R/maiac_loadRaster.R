...
maiac_loadRaster <- function(tileNumber = NULL,
                             date = NULL, 
                             time = NULL,
                             julianDate = NULL,
                             product = "MAIACAAOT",
  ...
  if ( any(params %in% meta$grid_1km$vars) && any(params %in% meta$grid_5km$vars) ) {
  ...
  templateRaster <- raster::raster(nrows = meta$ydim, ncol = meta$xdim,
