...
if ( interactive() ) {
  ...
  option_list <- list(
    ...
    make_option(
      c("-c", "--cellSize"),
      default=10,
      help="Cell size (5km, 10km, or 20km) [default=\"%default\"]"
    ),
    make_option(
      c("-r", "--frameRate"),
      default=5,
...
logger.setup(
  traceLog = file.path(opt$logDir, "createRasterVideo_TRACE.log"),
  debugLog = file.path(opt$logDir, "createRasterVideo_DEBUG.log"), 
  infoLog  = file.path(opt$logDir, "createRasterVideo_INFO.log"), 
  errorLog = file.path(opt$logDir, "createRasterVideo_ERROR.log")
)
...
# Start logging
logger.info("Running createRasterVideo_exec.R version %s", VERSION)
  ...
  for (hour in hours) {
    ...
    for (scanFilename in scanFilenames) {
      ...
      if ("try-error" %in% class(result)) {
        ...
        par(bg = 'gray')
        plot(rstr[[opt$variable]], main =  paste(frameLocalTimestamp, "PDT"),
             col = pal_aod(9),
